 $(function(){
     ...
     $("#update").on('click',function() {
         ...
         if(numberId.length == 1) {
             ...
             $.get("/author/selectOnebyId",{"id":numberId[0]},function(data) {
                 var dom = [];
             ...
             $("#modalForm").on("submit", function(ev) {
                 var name = $("#name").val();
                 $.get("/author/selectRe",{"domain.name":name},function(data) {
                     if(data.length == "") {
                         $.post("/author/updateOne",$('#modalForm').serialize(),function(data){
     ...
     $("#addModal").on('click',function() {
         ...
         $("#modalForm").on("submit", function(ev) {
             ...
             $.get("/author/selectRe",{"domain.name":name},function(data) {
                 if(data.length == "") {
                     $.post("/author/insertOne",$('#modalForm').serialize(),function(data){
                         $('#Modal').modal('hide');search();
                     });
                 }else{
                     $("#error").empty().text('标签已存在').css('color','#d01a1a');
     ...
     $("#delect").on('click',function() {
         ...
         if(ids.length != 0) {
              if(confirm("确认删除？")) {
                  $.post("/author/updateStateManyOrOne",{"ids":JSON.stringify(ids)},function(data) {
                      search();
                  });
              }
