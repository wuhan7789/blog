 $(function(){
     ...
     $("#update").on('click',function() {
         ...
         if(numberId.length == 1) {
             ...
             $.get("/tags/selectOnebyId",{"id":numberId[0]},function(data) {
                 var dom = [];
             ...
             $("#modalForm").on("submit", function(ev) {
                 var name = $("#name").val();
                 $.get("/tags/selectRe",{"domain.name":name},function(data) {
                     if(data.length == "") {
                         $.post("/tags/updateOne",$('#modalForm').serialize(),function(data){
     ...
     $("#addModal").on('click',function() {
         ...
         $("#modalForm").on("submit", function(ev) {
             ...
             $.get("/tags/selectRe",{"domain.name":name},function(data) {
                 if(data.length == "") {
                     $.post("/tags/insertOne",$('#modalForm').serialize(),function(data){
                         $('#Modal').modal('hide');search();
                     });
                 }else{
                     $("#error").empty().text('标签已存在').css('color','#d01a1a');
     ...
     $("#delect").on('click',function() {
         ...
         if(ids.length != 0) {
              if(confirm("确认删除？")) {
                  $.post("/tags/updateStateManyOrOne",{"ids":JSON.stringify(ids)},function(data) {
                      search();
                  });
              }
            ...
            success: function (data) {
                if (data.rows != "") {
                    ...
                    for(var i = 0 ; i < data.rows.length; i++) {
                        ...
                        dom.push("</td>");
                        dom.push("<td>");
                        dom.push(data.rows[i].domain.articleTotalNum);
                        dom.push("</td>");
                        dom.push("</tr>");
                    }
